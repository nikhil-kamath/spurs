type compressed_storage = CSR | CSC [@@deriving show]
...
(** Compressed matrix in the CSR or CSC format, with sorted indices.
    ...
    knowledge of its internals. For easier matrix construction, use the triplet
    format.
    ...
    In the compressed storage format, the non-zero values of a sparse matrix are
    stored as the row and column location of the non-zero values, with a
    ...
    `nnz = indptr[nrows]`. This index pointer array can then be used to
    ...
    front, the [triplet format](struct.TriMatBase.html) can be used. A matrix in
    the triplet format can then be efficiently converted to a `CsMat`. *)
...
type 'a cs_vec_base = { dim : int; indices : int array; data : 'a array }
...
module Utils = struct
  ...
  let check_compressed_structure (inner : int) (outer : int)
      (indptr : int array) (indices : int array) : (unit, string) Result.t =
    let open Result in
    let ( let* ) = bind in
    let* () =
    ...
    let* () =
      if
        Indptr.fold_outer indptr
          (fun s e a -> a && sorted_indices indices s e)
          true
      then ok ()
